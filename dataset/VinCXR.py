...
class DatasetGenerator(Dataset):
        ...
        for file_path in path_to_dataset_file:
            with open(file_path, "r") as f:
              for line in f: 
                    ...
                    if is_test==False:
                        gtlabels.append(items[2])#class_label
                        gtboxes.append(items[3])#gtbox
    ...
    def __getitem__(self, index):
        ...
        if is_test==False:
            ...
            gtlabel = self.gtlabels[index]
            box = self.gtboxes[index]
            x_scale = config['TRAN_SIZE']/width
            y_scale = config['TRAN_SIZE']/height
            crop_del = (config['TRAN_SIZE']-config['TRAN_CROP'])/2
            x_min, y_min = int(box[0])*x_scale-crop_del, int(box[1])*y_scale-crop_del
            x_max, y_max = int(box[2])*x_scale-crop_del, int(box[3])*y_scale-crop_del
...
def DicomProcess():
    ...
    trainset = pd.concat([X_train, y_train], axis=1).to_csv('/data/pycode/CXRAD/VinCXR_train.txt', index=False, header=False, sep=',')
    valset = pd.concat([X_val, y_val], axis=1).to_csv('/data/pycode/CXRAD/VinCXR_val.txt', index=False, header=False, sep=',')
    ...
    test_set.to_csv('/data/pycode/CXRAD/VinCXR_test.txt', index=False, header=False, sep=',')
